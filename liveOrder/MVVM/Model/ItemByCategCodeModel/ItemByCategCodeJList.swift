...
public final class ItemByCategCodeJList: NSCoding {
    ...
    // MARK: Properties
    public var acThumbnailImages: [ItemByCategCodeAcThumbnailImages]?
    ...
    public required init(json: JSON) {
      if let items = json[SerializationKeys.acThumbnailImages].array { acThumbnailImages = items.map { ItemByCategCodeAcThumbnailImages(json: $0) } }
    ...
    required public init(coder aDecoder: NSCoder) {
      self.acThumbnailImages = aDecoder.decodeObject(forKey: SerializationKeys.acThumbnailImages) as? [ItemByCategCodeAcThumbnailImages]
