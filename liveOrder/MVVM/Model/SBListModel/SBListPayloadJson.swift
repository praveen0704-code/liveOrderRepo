...
public final class SBListPayloadJson: NSCoding {
  ...
  public required init(json: JSON) {
    if let items = json[SerializationKeys.data].array { data = items.map { SBListData(json: $0) } }
  ...
  required public init(coder aDecoder: NSCoder) {
    self.data = aDecoder.decodeObject(forKey: SerializationKeys.data) as? [SBListData]
