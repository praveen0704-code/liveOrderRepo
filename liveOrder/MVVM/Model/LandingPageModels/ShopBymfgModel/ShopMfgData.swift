...
public final class ShopMfgData: NSCoding {
  ...
  public required init(json: JSON) {
    if let items = json[SerializationKeys.jList].array { jList = items.map { ShopMfgJList(json: $0) } }
  ...
  required public init(coder aDecoder: NSCoder) {
    self.jList = aDecoder.decodeObject(forKey: SerializationKeys.jList) as? [ShopMfgJList]
